...
import { syncUserData, invalidateUserCache } from "../services/dataSync";
import { generateTasksFromContext } from "../services/taskGenerator";
...
export async function getHomePage(req: AuthRequest, res: Response) {
  try {
    ...
    if (userData.emails.length > 0) {
      const { IntegrationModel } = await import("../models/Integrations");
    ...
    // Generate tasks from all the data
    const tasks = await generateTasksFromContext(
      {
        emails: userData.emails,
        calendarEvents: userData.calendarEvents,
        slackMessages: userData.slackMessages,
        linearIssues: userData.linearIssues,
        githubNotifications: userData.githubNotifications,
        githubIssues: userData.githubIssues,
        githubPRs: userData.githubPRs,
      },
      userName
    );
